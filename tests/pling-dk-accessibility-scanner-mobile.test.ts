...
test('Accessibility scan for all URLs', async ({ browser }) => {
    ...
    await page.waitForSelector('button.m-menu-toggler', { timeout: 10000 });
    const burgerMenu = await page.$('button.m-menu-toggler');
    if (burgerMenu) {
        console.log('Clicking Burger menu...');
        await Promise.all([
    ...
    await page.waitForSelector('a.menu-manager__appear-item', { timeout: 10000 });
    if (burgerMenu) {
        console.log('Clicking Burger menu...');
        await Promise.all([
            page.waitForLoadState('domcontentloaded'),
            await page.click('a:has-text("Min Konto")')
        ]);
        console.log('Min Konto clicked.');
    } else {
        console.log('Min Konto not found.');
    }
...
test.afterAll(async () => {
    ...
    writeHtmlReport(grouped, urls, 'artifacts', `pling-dk-accessibility-report-${process.env.NODE_ENV!}-mobile.html`);
});
