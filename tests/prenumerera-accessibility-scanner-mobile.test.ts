...
test('Accessibility scan for all URLs', async ({ browser }) => {
    ...
    await page.screenshot({ path: 'artifacts/prenumerera-mobile-start-page.png', fullPage: true });
    ...
    console.log('Waiting for "Genvägar" shortcut grid title...');
    await page.waitForSelector('h4.shortcut-grid__title.mb-5', { timeout: 500000 });
    console.log('"Genvägar" shortcut grid loaded.');
...
test.afterAll(async () => {
    ...
    writeHtmlReport(grouped, urls, 'artifacts', `prenumerera-accessibility-report-${process.env.NODE_ENV!}-mobile.html`);
});
